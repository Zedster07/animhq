...
window.cmb = (function (window, document, $, undefined) {
  ...
  cmb.init = function () {
     ...
     */
    cmb.initPickers(
      $metabox.find("input:text.cmb_timepicker"),
      $metabox.find("input:text.cmb_datepicker"),
      $metabox.find("input:text.cmb_colorpicker")
    );
    ...
    $(
      '<p><span class="button cmb-multicheck-toggle">' +
        l10n.check_toggle +
        "</span></p>"
  ...
  cmb.resetTitlesAndIterator = function () {
    ...
    $(".repeatable-group").each(function () {
      ...
      $table.find(".repeatable-grouping").each(function (rowindex) {
        ...
        // Reset rows title
        $row.find(".cmb-group-title h4").text(
          $table
            .find(".add-group-row")
            .data("grouptitle")
            .replace("{#}", rowindex + 1)
        ...
        $(attachment).each(function () {
          if (this.type && this.type === "image") {
            ...
            uploadStatus =
              ...
              this.filename +
              '">' +
              '<p><a href="#" class="cmb_remove_file_button" rel="' +
              cmb.formfield +
              "[" +
              this.id +
              ']">' +
            ...
            uploadStatus =
              ...
              l10n.download +
              '</a> / <a href="#" class="cmb_remove_file_button" rel="' +
              cmb.formfield +
              "[" +
              this.id +
              ']">' +
  ...
  cmb.handleRemoveMedia = function (event) {
    ...
    cmb
      .metabox()
      .find("input#" + cmb.formfield)
      .val("");
    cmb
      .metabox()
      .find("input#" + cmb.formfield + "_id")
      .val("");
    if (!$container.length) {
  ...
  $.fn.cleanRow = function (prevNum, group) {
    ...
    if ($self.find(".cmb-group-title")) {
      $self
        .find(".cmb-group-title h4")
        .text($self.data("title").replace("{#}", cmb.idNumber + 1));
    }
  ...
  cmb.afterRowInsert = function ($row) {
    ...
    cmb.initPickers(
      $row.find("input:text.cmb_timepicker"),
      $row.find("input:text.cmb_datepicker"),
      $row.find("input:text.cmb_colorpicker")
    );
  };
...
//     event.preventDefault();
  ...
  cmb.shiftRows = function (event) {
    ...
    var $self = $(this);
    var $parent = $self.parents(".repeatable-grouping");
    var $goto = $self.hasClass("move-up")
  ...
  cmb.initColorPickers = function ($selector) {
    ...
    if (
      typeof jQuery.wp === "object" &&
      typeof jQuery.wp.wpColorPicker === "function"
    ) {
