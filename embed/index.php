...
if(isset($_GET['vh'])){    try {
    ...
    if($dbh != null){
        ...
        if( isset($_GET['vh'])){
            $gt = $dbh->prepare("SELECT * FROM links where slug = ?");
            $gt->execute(array($_GET['vh']));
            if ($gt->rowCount() != 0){
              $link = $gt->fetch(PDO::FETCH_ASSOC)["lurl"];
              $link = "http://animehqstream.ddns.net:8000/?video=".$link;
            }
